package com.softserve.edu.registrator.pages.resource;

import org.openqa.selenium.WebElement;

import com.softserve.edu.registrator.pages.common.ACommonComponent;
import com.softserve.edu.registrator.tools.search.Search;

public class AddResourcePage extends ACommonComponent {

	public AddResourcePage() {
		super();
	}

	private static int paramNumber = 1;

	public static int getParamNumber() {
		return paramNumber;
	}

	public static void setParamNumber(int paramNumber) {
		AddResourcePage.paramNumber = paramNumber;
	}

	private static final String OWNER_FIELD_ID = "owner_search";
	private static final String OBJECT_NAME_ID = "w-input-search";
	private static final String SUBCLASS_SELECT_ID = "resourcesTypeSelect";
	private static final String PROCURATIONS_ID = "delivery";
	private static final String POINTS_BUTTON_XPATH = "//a[contains(@data-target,'#points')]";
	private static final String POINT_NUMBER_ID = "myparam" + paramNumber;
	private static final String PERIMETER_AREA_XPATH = "//*[@id='calculatedParamsTab']";
	private static final String DISCRETE_VALUE_ID = "discreteValue";
	private static final String DISCRETE_COMMENT_ID = "discreteComment";
	private static final String LINEAR_VALUE_ID = "linearBegin";
	private static final String LINEAR_COMMENT_ID = "linearEnd";
	private static final String MAP_BUTTON_XPATH = "//a[contains(@data-target,'#map')]";
	private static final String ALL_UKRAINE_CHECKBOX_ID = "allUkraine";
	private static final String SAVE_BUTTON_ID = "submitForm";

	// Getters

	public WebElement getOwnerField() {
		return Search.id(OWNER_FIELD_ID);
	}

	public WebElement getObjectName() {
		return Search.id(OBJECT_NAME_ID);
	}

	public WebElement getSubclassSelect() {
		return Search.id(SUBCLASS_SELECT_ID);
	}

	public WebElement getProcurations() {
		return Search.id(PROCURATIONS_ID);
	}

	public WebElement getPointsButton() {
		return Search.xpath(POINTS_BUTTON_XPATH);
	}

	public WebElement getPointNumber() {
		return Search.id(POINT_NUMBER_ID);
	}

	public WebElement getPerimeterArea() {
		return Search.xpath(PERIMETER_AREA_XPATH);
	}

	public WebElement getDiscreteValue() {
		return Search.id(DISCRETE_VALUE_ID);
	}

	public WebElement getDiscreteComment() {
		return Search.id(DISCRETE_COMMENT_ID);
	}

	public WebElement getLinearValue() {
		return Search.id(LINEAR_VALUE_ID);
	}

	public WebElement getLinearComment() {
		return Search.id(LINEAR_COMMENT_ID);
	}

	public WebElement getMapButton() {
		return Search.xpath(MAP_BUTTON_XPATH);
	}

	public WebElement getAllUkraineCheckbox() {
		return Search.id(ALL_UKRAINE_CHECKBOX_ID);
	}

	public WebElement getSaveButton() {
		return Search.id(SAVE_BUTTON_ID);
	}

	// Clicks

	public void clickOwnerField() {
		getOwnerField().click();
	}

	public void clickObjectName() {
		getObjectName().click();
	}

	public void clickSubclassSelect() {
		getSubclassSelect().click();
	}

	public void clickProcurations() {
		getProcurations().click();
	}

	public void clickPointsButton() {
		getPointsButton().click();
	}

	public void clickPerimeterArea() {
		getPerimeterArea().click();
	}

	public void clickDiscreteValue() {
		getDiscreteValue().click();
	}

	public void clickDiscreteComment() {
		getDiscreteComment().click();
	}

	public void clickLinearValue() {
		getLinearValue().click();
	}

	public void clickLinearComment() {
		getLinearComment().click();
	}

	public void clickMapButton() {
		getMapButton().click();
	}

	public void clickAllUkraineCheckbox() {
		getAllUkraineCheckbox().click();
	}

	public void clickSaveButton() {
		getSaveButton().click();
	}

	public void clickResourceByName(String name) {
		clickSubclassSelect();
		Search.cssSelector("*[value='" + name + "']").click();
	}

	// Fills

	public void fillOwnerField(String owner) {
		clickOwnerField();
		getOwnerField().sendKeys(owner);
	}

	public void fillObjectName(String object) {
		clickObjectName();
		getObjectName().sendKeys(object);
	}

	public void fillDiscreteComment(String comment) {
		clickDiscreteComment();
		getDiscreteComment().sendKeys(comment);
	}

	public void fillDiscreteValue(String value) {
		clickDiscreteValue();
		getDiscreteValue().sendKeys(value);
	}

	public void fillLinearValue(String value) {
		clickLinearValue();
		getLinearValue().sendKeys(value);
	}

	public void fillLinearComment(String comment) {
		clickLinearComment();
		getLinearComment().sendKeys(comment);
	}

	//
	// Functions
	//
	//
	//
	//
	//

	public void fillPoints() {
		Search.id("myparam1").click();
		Search.id("myparam1").clear();
		Search.id("myparam1").sendKeys("1");
		Search.id("myparam2").click();
		Search.id("myparam2").clear();
		Search.id("myparam2").sendKeys("1");
		Search.id("myparam3").click();
		Search.id("myparam3").clear();
		Search.id("myparam3").sendKeys("1.1");
		Search.id("myparam4").click();
		Search.id("myparam4").clear();
		Search.id("myparam4").sendKeys("1");
		Search.id("myparam5").click();
		Search.id("myparam5").clear();
		Search.id("myparam5").sendKeys("1");
		Search.id("myparam6").click();
		Search.id("myparam6").clear();
		Search.id("myparam6").sendKeys("1.1");
		Search.id("btnAddAreaPoint").click();
		Search.cssSelector("#areaInput2 #myparam1").click();
		Search.cssSelector("#areaInput2 #myparam1").clear();
		Search.cssSelector("#areaInput2 #myparam1").sendKeys("2");
		Search.cssSelector("#areaInput2 #myparam2").click();
		Search.cssSelector("#areaInput2 #myparam2").clear();
		Search.cssSelector("#areaInput2 #myparam2").sendKeys("2");
		Search.cssSelector("#areaInput2 #myparam3").click();
		Search.cssSelector("#areaInput2 #myparam3").clear();
		Search.cssSelector("#areaInput2 #myparam3").sendKeys("2.1");
		Search.cssSelector("#areaInput2 #myparam4").click();
		Search.cssSelector("#areaInput2 #myparam4").clear();
		Search.cssSelector("#areaInput2 #myparam4").sendKeys("2");
		Search.cssSelector("#areaInput2 #myparam5").click();
		Search.cssSelector("#areaInput2 #myparam5").clear();
		Search.cssSelector("#areaInput2 #myparam5").sendKeys("2");
		Search.cssSelector("#areaInput2 #myparam6").click();
		Search.cssSelector("#areaInput2 #myparam6").clear();
		Search.cssSelector("#areaInput2 #myparam6").sendKeys("2.1");
		Search.id("btnAddAreaPoint").click();
		Search.cssSelector("#areaInput3 #myparam1").click();
		Search.cssSelector("#areaInput3 #myparam1").clear();
		Search.cssSelector("#areaInput3 #myparam1").sendKeys("3");
		Search.cssSelector("#areaInput3 #myparam2").click();
		Search.cssSelector("#areaInput3 #myparam2").clear();
		Search.cssSelector("#areaInput3 #myparam2").sendKeys("3");
		Search.cssSelector("#areaInput3 #myparam3").click();
		Search.cssSelector("#areaInput3 #myparam3").clear();
		Search.cssSelector("#areaInput3 #myparam3").sendKeys("3.1");
		Search.cssSelector("#areaInput3 #myparam4").click();
		Search.cssSelector("#areaInput3 #myparam4").clear();
		Search.cssSelector("#areaInput3 #myparam4").sendKeys("3");
		Search.cssSelector("#areaInput3 #myparam5").click();
		Search.cssSelector("#areaInput3 #myparam5").clear();
		Search.cssSelector("#areaInput3 #myparam5").sendKeys("3");
		Search.cssSelector("#areaInput3 #myparam6").click();
		Search.cssSelector("#areaInput3 #myparam6").clear();
		Search.cssSelector("#areaInput3 #myparam6").sendKeys("3.1");
	}

}
